...
pyr_df_clean$arm <- as.factor(pyr_df_clean$arm)
sulf_df_clean$arm <- as.factor(sulf_df_clean$arm)

#pdf("plots/scatter_drug_time.pdf", height = 4, width = 7)
#op <- par(mfrow = c(1,2))
#with(pyr_df_clean,  plot(pday, Pyrconcentration,  pch = 16, cex = .6, col = as.integer(arm),
#                   main = "Pyrimethamine (raw)", xlab = "Day", ylab = #"Concentration"))
#legend("topright", legend = levels(pyr_df_clean$arm), pch = 16, col = #seq_along(levels(pyr_df_clean$arm)), bty = "n")

#with(sulf_df_clean, plot(pday, Sulfconcentration, pch = 16, cex = .6, col = as.integer(arm),
#                   main = "Sulfadoxine (raw)", xlab = "Day", ylab = "Concentration"))
#legend("topright", legend = levels(sulf_df_clean$arm), pch = 16, col = seq_along(levels(sulf_df_clean$arm)), bty = "n")
#par(op)
#dev.off()
