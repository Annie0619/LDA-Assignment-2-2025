...
pdf("plots/qq_lke_base.pdf", width = 4, height = 3.5)
...
```{r}
## 1) Subject-level random effects + covariates 
re    <- ranef(m1_ML) %>% as.data.frame()     
...
subj <- dat_re %>%
  group_by(pid) %>%
  summarise(
    b_lKe    = first(na.omit(lKe)),   # (random effect) for lKe
...
## 2) Continuous: random effects vs c_weight / c_age 
cont_long <- subj %>%
...
p_cont <- ggplot(cont_long, aes(x = x, y = b_lKe)) +
  ...
  theme_bw() +
  labs(title = "lKe vs continuous covariates",
       x = "Covariate value", y = "lKe")
p_cont
ggsave("plots/age_weight_lke.pdf", plot = p_cont, height = 3, width = 5)

## 3) Categorical: BLUP by gender / arm / region
g_gender <- ggplot(filter(subj, !is.na(gender)),
    aes(x = as.factor(gender), y = b_lKe)) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_bw() +
  labs(title = "lKe by gender", x = "gender", y = "lKe")
ggsave("plots/gender_lke.pdf", plot = g_gender, height = 3, width = 3)

g_arm <- ggplot(filter(subj, !is.na(arm)),
    aes(x = as.factor(arm), y = b_lKe)) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_bw() +
  labs(title = "lKe by arm", x = "arm", y = "lKe")
ggsave("plots/arm_lke.pdf", plot = g_arm, height = 3, width = 3)

g_region <- ggplot(filter(subj, !is.na(region)),
        aes(x = as.factor(region), y = b_lKe)) +
  geom_boxplot(outlier.alpha = 0.5) +
  theme_bw() +
  labs(title = "lKe by region", x = "region", y = "lKe")
g_region
ggsave("plots/region_lke.pdf", plot = g_region, height = 3, width = 3)
```
...
Determine if we should keep age or weight.

```{r}
AIC(m2a_ML, m2w_ML)
BIC(m2a_ML, m2w_ML)

VarCorr(m2w_ML) # variance of lKe got smaller since we explained part of it.
VarCorr(m1_ML)
```
...
```{r}
m_final <- update(m2w_ML, method = "REML")
summary(m_final)
...
# Residual plots
pdf("plots/resid_fitted_base_weight.pdf", width = 4, height = 4)
plot(m_final)  # residuals vs fitted
dev.off()
pdf("plots/qq_base_weight.pdf", width = 4, height = 4)
...
res <- resid(m_final, type = "normalized")                 # length = nobs(model)
pid <- as.character(getGroups(m_final, level = 1))         # same length as res
...
# Residuals by arm
g_arm <- ggplot(df_res, aes(x = arm, y = res)) +
  geom_boxplot() +
  theme_bw() +
  labs(title = "Residuals by arm", x = "Arm", y = "Normalized residuals")
ggsave("plots/residual_by_arm.pdf", plot = g_arm, height = 3, width = 3)

# Residuals by region
g_region <- ggplot(df_res, aes(x = region, y = res)) +
  geom_boxplot() +
  theme_bw() +
  labs(title = "Residuals by region", x = "Region", y = "Normalized residuals")
ggsave("plots/residual_by_region.pdf", plot = g_region, height = 3, width = 3)

# Residuals by gender
g_gender <- ggplot(df_res, aes(x = gender, y = res)) +
  geom_boxplot() +
  theme_bw() +
  labs(title = "Residuals by gender", x = "Gender", y = "Normalized residuals")
ggsave("plots/residual_by_gender.pdf", plot = g_gender, height = 3, width = 3)
```

No clear difference. Does variance increase with weight?

```{r}
# Subject-level covariate: weight
covs <- as.data.frame(pyr_gd) %>%
  group_by(pid) %>%
  summarise(
    c_weight = first(na.omit(c_weight)),
...
# Join residuals to covariates
df_res <- df_used %>% left_join(covs, by = "pid")

# Plot residuals vs weight
g_resid_weight <- ggplot(df_res, aes(x = c_weight, y = res)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  theme_bw() +
  labs(
    title = "Normalized residuals vs. weight",
    x = "Weight",
    y = "Normalized residuals"
  )
ggsave("plots/residual_by_weight.pdf", plot = g_resid_weight, height = 3, width = 3)
```
...
```{r}
pdf("plots/acf_base_weights.pdf", height = 3, width = 4)
plot(ACF(m_final, maxLag =10), alpha = 0.05)
dev.off()
```
...
```{r}
## Fit candidates (ML) 
m_base <- m_final  # current ML model (no correlation)
# specify controls to use if needed:
ctl <- lmeControl(maxIter = 2000, msMaxIter = 2000, tolerance = 1e-6)
...
# did not converge, even with changed controls:
m_ma1  <- update(
  m_base,
...
## Compare by AIC
AIC(m_base, m_car1, m_ar1,
    m_ar2, m_ma2, m_arma12, m_arma21)

## For nested pairs do LRTs; 
...
pdf("plots/acf_ar2.pdf", height = 3, width = 4)
plot(ACF(m_ar2,   maxLag = 10), alpha = 0.05, main = "AR(2)")
dev.off()
pdf("plots/acf_ma2.pdf", height = 3, width = 4)
plot(ACF(m_ma2,   maxLag = 10), alpha = 0.05, main = "MA(2)")
dev.off()
pdf("plots/acf_arma12.pdf", height = 3, width = 4)
plot(ACF(m_arma12,   maxLag = 10), alpha = 0.05, main = "ARMA(1,2)")
dev.off()
...
# this model did not converge with all three random effects
# drop to only lKe:
...
summary(m_log1_ML)
# random effect standard deviation almost zero
# let's try adding covariates anyways

beta0 <- unname(fixef(m_log1_ML))  # starting values

# add same covariates as in the model without logging:
m_log_cov_ML <- update(
  m_log1_ML,
  fixed = list(
    lKe ~ 1 + c_weight + I(c_weight^2), 
  ...
  start = c(
    beta0[1], 0, 0,   # lKe intercept + slopes
    beta0[2],           # lKa intercept
    beta0[3]            # lCl intercept
  )
)
summary(m_log_cov_ML) # none of the covariates are significant!
anova(m_log1_ML, m_log_cov_ML) # p-val 0.96- should not add covariates


# refit the basic model with REML
m_log_final <- update(m_log1_ML, method = "REML")
summary(m_log_final) # standard deviation of lke random effect almost zero!

pdf("plots/resid_fitted_log.pdf", height = 4, width = 4)
plot(m_log_final) # residuals very weird.. vertical lines at discrete points.
dev.off()

pdf("plots/qq_log.pdf", height = 4, width = 4)
qqnorm(resid(m_log_final, type = "normalized")); qqline(resid(m_log_final, type = "normalized"))
dev.off()
# qq plot also very weird with sections of breaks and horizontal dots

hist(resid(m_log_final, type = "normalized")) # very clear dual peaks

AIC(m_final, m_log_final) # log model much lower AIC
```

# Fitted prediction curves

```{r}
dat <- getData(m2w_ML) %>% as.data.frame()

# Add fitted values: level=1 uses subject BLUPs, level=0 is population
dat$fit_subj <- predict(m2w_ML, level = 1)
dat$fit_pop  <- predict(m2w_ML, level = 0)

set.seed(70)
some_ids <- sample(unique(dat$pid), 9)   # pick 16 to show
g_fitted <- ggplot(filter(dat, pid %in% some_ids),
       aes(pday, Pyrconcentration, colour = "Observed")) +
  geom_point(alpha = 0.6, size = 1.2) +
  geom_line(aes(y = fit_subj, colour = "Fitted (level=1)"),
            linewidth = 0.7) +
  scale_colour_manual(values = c("Observed" = "black", "Fitted (level=1)" = "steelblue")) +
  theme_bw() + facet_wrap(~ pid, scales = "free_y") +
  labs(x = "Time (days)", y = "Concentration", colour = NULL,
       title = "Observed and fitted by subject (subset)")
ggsave("plots/fitted_curves.pdf", plot = g_fitted, width = 7, height = 5)
```
